<?php 
    $web_title = $san_pham->name;
    $web_description = $san_pham->description;
    $web_image = productImageFolder().$san_pham->image;
?>
...
>>>>>>> 2edb44b0b68c847c9af9804e9245765302d73f8d
            ...
            <div class="col-md-9 col-sm-8">
                <div class="main-wrapper">
                    <div class="intro-panel">
                        <h1 class="detail-mk">{{$san_pham->name}}</h1>
                        <div class="row">
                            <div class="col-md-7">
                                <div class="product-img">
                                    <a class="gallery-item" title="{{$san_pham->name}}" href="{{URL::to(!empty($san_pham->image) ? productImageFolder().$san_pham->image : '/images/no_image.jpg')}}">
                                    {{HTML::image(!empty($san_pham->image) ? productImageFolder().$san_pham->image : "/images/no_image.jpg",$san_pham->name)}}
                                    </a>
                                </div>
                                ...
                                <a class="gallery-item" title="{{$image->name}}" href="{{URL::to(productImageFolder().$image->url)}}">
                                    {{HTML::image(productThumbImageFolder().$image->url,$image->name)}}
                                </a>
                                @endforeach                    
                            </div>
<<<<<<< HEAD
...
>>>>>>> 2edb44b0b68c847c9af9804e9245765302d73f8d
                        ...
                        <div class="product-content">
                            ...
                            @if(!empty($san_pham->feature))
                                <h2 class="content-title">
                                    Đặc trưng sản phẩm
                                </h2>
                                {{$san_pham->feature}}
                            @endif
                            @if(!empty($san_pham->content))
                            <h2 class="content-title">
                                    Thông tin chi tiết
                                </h2>
                            {{$san_pham->content}}
                        ...
                        <div class="cam-ket">
                            <h2>Cam kết của chúng tôi</h2>
                            <p>
                                • MATKINHDANANG.COM cam kết không bán sản phẩm kém chất lượng, tất cả hình ảnh sản phẩm đều là do chúng tôi tự chụp từ chính sản phẩm thật, quý khách sẽ mua được chính xác những gì đang xem.
                                <br> • Quý khách được miễn phí vận chuyển và kiểm tra sản phẩm trước khi thanh toán.
                                <br> • Ngoài ra MATKINHDANANG.COM có chương trình ưu đãi đặc biệt ReBuying dành cho khách hàng thành viên. Quý khách có thể xem chi tiết tại đây
                                <br>
                            </p>
                        </div>
                        <div class="related-product">
